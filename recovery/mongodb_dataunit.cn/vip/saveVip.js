...
async function saveVip() {
  ...
  for (let i = 0; i < user_1.length; i++) {
    ...
    const { _id, money, isYearCard, restTotal, usedTotal, total, birthday, createTime, updateTime, id, cardId, cardType, name, remark, overdate, phone, sex } = item
    const key = _id + restTotal.toString() + createTime + updateTime
    // if (repet[key]) {
    //   data.push(item)
    // } else {
    //   repet[key] = true
    ...
    //   console.log(item)
    // }
    const url = 'http://127.0.0.1:7006/api/saveVip'
